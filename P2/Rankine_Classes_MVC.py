...
class rankineController():
    def __init__(self, *args):
        ...
        """
        self.Model = rankineModel() #NEW # In MVC pattern, the controller should hold a reference to the model
        self.View = rankineView() #NEW  # In MVC pattern, the controller should hold a reference to the view
        self.IW = args[0]  # an array of widgets that are for user input
        self.DW = args[1]  # an array of widgets that are for display
        self.View.setWidgets(self.IW, self.DW) #NEW # call the setWidgets function of the view with self.IW and self.DW as arguments (see unpacking)
